...
#pragma omp parallel for
    for (unsigned int i = 0; i < size; ++i)
    {
        ...
        {
            result[i] += A[get_idx_matr(i, j, size)] * b[j];
        }
    }
}
...
{
    vector resid_vector(new double[size]);
    vector p_vector(new double[size]);
    vector buffer(new double[size]);
...
int main()
{
    ...
    for (unsigned int idx = 0; idx < iterations; ++idx)
    {
        ...
        matrix matrix(new double[num_elements * num_elements]{0});
