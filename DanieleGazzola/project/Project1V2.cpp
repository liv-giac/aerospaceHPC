...
//--------------------------------------THOMAS ALGORITHM--------------------------------------//
void thomas_x(const double exdiag, const int rank, const int size, const int start, const int end, const int direction, const double x)
{
    ...
    for (int i = local_start_z; i <= local_end_z; i++)
    {
        for (int j = local_start_y; j <= local_end_y; j++)
        {
            ...
            if (coord[direction] != 0)
            {
                ...
                MPI_Recv(&diag[local_start_x + j * local_size_x + i * local_size_x * local_size_y], 1, MPI_DOUBLE, prev_process, 2, comm2D, MPI_STATUS_IGNORE);
            }

            for (int k = local_start_x; k <= local_end_x; k++)
            {
                if (k != local_start_x || coord[direction] != 0)
                {
                    w = exdiag / diag[k - 1 + j * local_size_x + i * local_size_x * local_size_y];
                    diag[k + j * local_size_x + i * local_size_x * local_size_y] -= w * exdiag;
            ...
            if (coord[direction] != dims[direction] - 1)
            {
                ...
                MPI_Send(&diag[local_end_x + j * local_size_x + i * local_size_x * local_size_y], 1, MPI_DOUBLE, next_process, 2, comm2D);
            }
        }
    }
    ...
    for (int i = local_start_z; i < local_end_z; i++)
    {
        for (int j = local_start_y; j < local_end_y; j++)
        {
            ...
            if (coord[direction] != dims[direction] - 1)
                ...
                local_sol[local_end_x + j * local_size_x + i * local_size_x * local_size_y] /= diag[local_end_x + j * local_size_x + i * local_size_x * local_size_y];

            for (int k = local_end_x; k >= local_start_x; k--)
            {
                if (k != local_end_x || coord[direction] != dims[direction] - 1)
                    local_sol[k + j * local_size_x + i * local_size_x * local_size_y] = (local_sol[k + j * local_size_x + i * local_size_x * local_size_y] - exdiag * next) / diag[k + j * local_size_x + i * local_size_x * local_size_y];
    ...
    //             {
    //                 std::cout << local_sol[i + j * local_size_x + k * local_size_x * local_size_y] << std::endl;
    //             }
    //             std::cout << std::endl;
    //         }
    //     }
...
void thomas_y(const double exdiag, const int rank, const int size, const int start, const int end, const int direction)
{
    ...
    for (int i = local_start_z; i <= local_end_z; i++)
    {
        for (int j = local_start_x; j <= local_end_x; j++)
        {
            ...
            for (int i = local_start_y; i <= local_end_y; i++)
                diag[i] = 1.0 + dt / dx2;

            if (coord[direction] != 0)
            {
                MPI_Recv(&prev, 1, MPI_DOUBLE, prev_process, 1, comm2D, MPI_STATUS_IGNORE);
                MPI_Recv(&diag[j + local_start_y * local_size_x + i * local_size_x * local_size_y], 1, MPI_DOUBLE, prev_process, 2, comm2D, MPI_STATUS_IGNORE);
            }

            for (int k = local_start_y; k <= local_end_y; k++)
            {
                if (k != local_start_y || coord[direction] != 0)
                {
                    w = exdiag / diag[j + (k - 1) * local_size_x + i * local_size_x * local_size_y];
                    diag[j + k * local_size_x + i * local_size_x * local_size_y] = diag[j + k * local_size_x + i * local_size_x * local_size_y] - w * exdiag;
            ...
            {
                MPI_Send(&prev, 1, MPI_DOUBLE, next_process, 1, comm2D);
                MPI_Send(&diag[j + local_end_y * local_size_x + i * local_size_x * local_size_y], 1, MPI_DOUBLE, next_process, 2, comm2D);
            }
        }
    }
    ...
    for (int i = local_start_z; i < local_end_z; i++)
    {
        for (int j = local_start_x; j < local_end_x; j++)
        {
            ...
            if (coord[direction] != dims[direction] - 1)
                ...
                local_sol[j + local_end_y * local_size_x + i * local_size_x * local_size_y] /= diag[j + local_end_y * local_size_x + i * local_size_x * local_size_y];

            for (int k = local_end_y; k >= local_start_y; k--)
            {
                if (k != local_end_y || coord[direction] != dims[direction] - 1)
                    local_sol[j + k * local_size_x + i * local_size_x * local_size_y] = (local_sol[j + k * local_size_x + i * local_size_x * local_size_y] - exdiag * next) / diag[j + k * local_size_x + i * local_size_x * local_size_y];
...
void thomas_z(const double exdiag, const int rank, const int size, const int start, const int end, const int direction)
{
    ...
    for (int j = local_start_y; j < local_end_y; j++)
    {
        for (int i = local_start_x; i < local_end_x; i++)
        {
            ...
            {
                MPI_Recv(&prev, 1, MPI_DOUBLE, prev_process, 1, comm2D, MPI_STATUS_IGNORE);
                MPI_Recv(&diag[i + j * local_size_x + local_start_z * local_size_x * local_size_y], 1, MPI_DOUBLE, prev_process, 2, comm2D, MPI_STATUS_IGNORE);
            }

            for (int k = local_start_z; k <= local_end_z; k++)
            {
                if (k != local_start_z || coord[direction] != 0)
                {
                    w = exdiag / diag[i + j * local_size_x + (k - 1) * local_size_x * local_size_y];
                    diag[i + j * local_size_x + k * local_size_x * local_size_y] = diag[i + j * local_size_x + k * local_size_x * local_size_y] - w * exdiag;
            ...
            {
                MPI_Send(&prev, 1, MPI_DOUBLE, next_process, 1, comm2D);
                MPI_Send(&diag[i + j * local_size_x + local_end_z * local_size_x * local_size_y], 1, MPI_DOUBLE, next_process, 2, comm2D);
            }
        }
    }
    ...
    for (int j = local_start_y; j < local_end_y; j++)
    {
        for (int i = local_start_x; i < local_end_x; i++)
        {
            ...
            if (coord[direction] != dims[direction] - 1)
                ...
                local_sol[i + j * local_size_x + local_end_z * local_size_x * local_size_y] /= diag[i + j * local_size_x + local_end_z * local_size_x * local_size_y];

            for (int k = local_end_z; k >= local_start_z; k--)
            {
                if (k != local_end_z || coord[direction] != dims[direction] - 1)
                    local_sol[i + j * local_size_x + k * local_size_x * local_size_y] = (local_sol[i + j * local_size_x + k * local_size_x * local_size_y] - exdiag * next) / diag[i + j * local_size_x + k * local_size_x * local_size_y];
...
void parallel_x_direction(const std::vector<int> &counts, const std::vector<int> &displs, const int start, const int end, const double dt, const double dx2, const int x)
{
    ...
    thomas_x(exdiag, rank, size, start, end, 0, x);
}
